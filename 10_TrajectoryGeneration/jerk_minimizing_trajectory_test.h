#ifndef JERK_MINIMIZING_TRAJECTORY_TEST_H_
#define JERK_MINIMIZING_TRAJECTORY_TEST_H_
...
bool close_enough(vector<double> &poly, vector<double> &target_poly, double eps=0.01)
{
	...
	{
		std::cout << "Your solution didn't have the correct number of terms" << std::endl;
		return false;
	}
	...
	for (int i = 0; i < poly.size(); ++i)
	{
		...
		if (abs(diff) > eps)
		{
			std::cout << "At least one of your terms differed from terget by more than " << eps << std::endl;
			return false;
		}
	}
	return true;
...
#endif // JERK_MINIMIZING_TRAJECTORY_TEST_H_